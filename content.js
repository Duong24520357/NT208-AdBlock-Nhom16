...
async function initialize() {
  try {
    ...
    if (studyBlocked && isTopFrame()) {
      ...
      console.log(
        "[AdBlock] Đã chặn truy cập (học tập):",
...
function hasStrongAdSignal(node) {
  ...
  const hasKnownToken =
    /(adsbygoogle|gpt-ad|div-gpt-ad|google_ads|banner-ad|advertisement|ad-container|ad-wrapper|sponsor|promo|video-ad|ad-video|under-player-ad|under-video-ad|outstream|preroll|midroll|postroll|inread|instream|no-ads-under|ads-under)/.test(
      text,
    );
...
function hideFloatingVideoAds() {
  ...
  const AD_CONTAINER_SELECTOR =
    ".no-ads-under,[class*='no-ads-under'],[id*='no-ads-under'],[class*='ads-under'],[id*='ads-under'],.plyr__video-wrapper,.plyr--video,[class*='video-overlay'],[class*='video-ad'],[class*='ad-video'],[class*='under-player-ad'],[class*='under-video-ad'],[class*='floating-video'],[class*='sticky-video'],[id*='video-ad'],[id*='ad-video']";
  ...
  videoContainers.forEach((element) => {
    ...
    const html =
      `${candidate.id || ""} ${candidate.className || ""} ${candidate.src || ""}`.toLowerCase();
    ...
    const isSmallFloating =
      rect.width < 500 && rect.width > 120 && rect.height > 80;
...
function hideVideoLikeAds() {
  ...
  specificSelectors.forEach((sel) => {
    try {
      ...
      /* selector không hợp lệ trên browser cũ */
    }
  });

  // --- 2. Heuristic: phân tích từng <video> ---
  document
    .querySelectorAll("video:not([data-adblock-hidden])")
    .forEach((video) => {
  ...
  document
    .querySelectorAll("iframe:not([data-adblock-hidden])")
    .forEach((iframe) => {
      const src = (
        iframe.src ||
...
function collapseEmptyAdPlaceholders() {
  ...
  placeholders.forEach((node) => {
    ...
    const hasVisibleChild = Array.from(node.children).some((child) => {
      if (
        child.dataset.adblockHidden === "true" ||
        child.dataset.adblockCollapsed === "true"
      ) {
        return false;
      }
...
function showAds() {
  const hiddenElements = document.querySelectorAll(
    "[data-adblock-hidden], [data-adblock-collapsed]",
  );
...
let fullPageStitchState = null;

function cleanupFullPageStitchState() {
  if (fullPageStitchState?.originalScrollBehavior !== undefined) {
    document.documentElement.style.scrollBehavior =
      fullPageStitchState.originalScrollBehavior;
  }
  fullPageStitchState = null;
}
...
async function waitForScrollSettled(targetY, maxAttempts = 12, waitMs = 35) {
  const target = Math.max(0, Math.floor(targetY || 0));
...
function formatCaptureFileName(hostname) {
  const safeHost = (hostname || "page")
    .toLowerCase()
    .replace(/[^a-z0-9.-]/g, "-")
    .replace(/-+/g, "-")
    .replace(/^-|-$/g, "");
  ...
  return `fullpage-${safeHost || "page"}-${timestamp}.png`;
}

async function exportCanvasAsDownload(canvas, fileName) {
  const blob = await new Promise((resolve) => canvas.toBlob(resolve, "image/png"));
  if (!blob) {
    throw new Error("BLOB_CREATE_FAILED");
  }
...
async function initFullPageStitch(payload) {
  if (!isTopFrame()) return { ok: false, reason: "NOT_TOP_FRAME" };

  const totalHeight = Math.max(1, Math.ceil(payload.totalHeight || 0));
  const viewportHeight = Math.max(1, Math.ceil(payload.viewportHeight || 0));
  const viewportWidth = Math.max(1, Math.ceil(payload.viewportWidth || 0));

  cleanupFullPageStitchState();
  fullPageStitchState = {
    totalHeight,
    viewportHeight,
    viewportWidth,
  ...
  // Một số site bật smooth scrolling toàn cục làm ảnh bị trùng đoạn khi chụp nhanh.
  document.documentElement.style.scrollBehavior = "auto";

  return { ok: true };
}

async function addFullPageStitchSegment(payload) {
  if (!isTopFrame()) return { ok: false, reason: "NOT_TOP_FRAME" };
  if (!fullPageStitchState) return { ok: false, reason: "NOT_INITIALIZED" };

  const image = await loadImageFromDataUrl(payload.dataUrl);

  if (!fullPageStitchState.canvas) {
    const guessedScale = image.width / fullPageStitchState.viewportWidth;
    const scale = Number.isFinite(guessedScale) && guessedScale > 0 ? guessedScale : 1;
    const canvas = document.createElement("canvas");
    canvas.width = image.width;
    canvas.height = Math.max(1, image.height);

    fullPageStitchState.canvas = canvas;
    fullPageStitchState.scale = scale;
  }

  let { canvas } = fullPageStitchState;
  const { scale } = fullPageStitchState;
  const y = Math.max(0, Math.floor(payload.y || 0));
  const drawPxHeight = Math.max(1, image.height);
  const dy = Math.round(y * scale);
  const sourceHeight = Math.min(drawPxHeight, image.height);
  const drawBottom = dy + sourceHeight;

  if (drawBottom > canvas.height) {
    const expandedCanvas = document.createElement("canvas");
    expandedCanvas.width = canvas.width;
    expandedCanvas.height = Math.max(drawBottom, canvas.height + image.height * 2);
    const expandedCtx = expandedCanvas.getContext("2d");
    expandedCtx.drawImage(canvas, 0, 0);
    fullPageStitchState.canvas = expandedCanvas;
  ...
  fullPageStitchState.maxDrawBottomPx = Math.max(
    fullPageStitchState.maxDrawBottomPx || 0,
    drawBottom,
  );
...
async function finalizeFullPageStitch() {
  if (!isTopFrame()) return { ok: false, reason: "NOT_TOP_FRAME" };
  if (!fullPageStitchState?.canvas) return { ok: false, reason: "NO_CAPTURE_DATA" };

  const sourceCanvas = fullPageStitchState.canvas;
  const cropHeight = Math.max(
    1,
    Math.min(
      sourceCanvas.height,
      Math.round(fullPageStitchState.maxDrawBottomPx || sourceCanvas.height),
    ),
  );
  ...
  const fileName = formatCaptureFileName(fullPageStitchState.hostname);
  await exportCanvasAsDownload(outputCanvas, fileName);
  cleanupFullPageStitchState();

  return { ok: true, fileName };
}

function abortFullPageStitch() {
  cleanupFullPageStitchState();
  return { ok: true };
}
...
function stopElementPicker(onMouseOver, onMouseClick, onKeyDown) {
  ...
  document
    .querySelectorAll(".adblock-highlight")
...
function shouldCollapseContainer(container) {
  ...
  const hasContentTags = container.querySelector(
    "article, main, p, h1, h2, h3, ul, ol, table, form",
  );
