...
(async () => {
  try {
    ...
    if (
      enabled &&
      window.StudyBlocker?.isHostnameBlocked?.(currentHostname, blockedDomains)
    ) {
...
const STRICT_AD_WRAPPER_SELECTOR =
  window.AdConstants?.STRICT_AD_WRAPPER_SELECTOR || "";

// hasStrongAdSignal delegated to blocking/ad-heuristics.js
function hasStrongAdSignal(node) {
  try {
    return window.AdHeuristics?.hasStrongAdSignal?.(node) || false;
  } catch {
    return false;
  }
}
...
function hideFloatingVideoAds() {
  try {
    return window.AdHeuristics?.hideFloatingVideoAds?.() || 0;
  } catch {
    return 0;
  }
}
...
function hideVideoLikeAds() {
  try {
    return window.AdHeuristics?.hideVideoLikeAds?.() || 0;
  } catch {
    return 0;
  }
}
