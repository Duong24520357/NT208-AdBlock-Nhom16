(async function() {
    ...
    if (!/(^|\.)studocu\.(com|vn)$/i.test(window.location.hostname)) return;
    try {
        const saved = await chrome.storage.local.get("adblockState");
        const enabled = saved?.adblockState?.enabled !== false;
        if (!enabled) return;
    } catch {
    ...
    function generatePDF() {
        ...
        if (!basePrefix || !totalPages) {
            alert('Adblock: Could not find page data. Try refreshing.');
            return;
        }
        ...
        // Studocu uses pdf2htmlEX which generates divs with dynamic classes like "c x19 y10 w8 h7"
        // nested inside a Viewer_descale container — standard selectors like [class^="pc"] miss these.
        var pfs = document.querySelectorAll('.pf');
        var pageOverlays = [];
        for (var p = 0; p < totalPages; p++) {
            var pf = pfs[p];
            if (!pf) { pageOverlays.push(''); continue; }

            var overlay = null;

            // Strategy 1: Viewer_descale — Studocu's React wrapper containing all dynamic text divs
            var viewerDescale = pf.querySelector('[class*="Viewer_descale"]');
            if (viewerDescale) {
                overlay = viewerDescale.cloneNode(true);
                overlay.querySelectorAll('img').forEach(function(img) { img.remove(); });
            }

            // Strategy 2: pdf2htmlEX pc-layer — match "pc" as a whole word via regex
            if (!overlay) {
                var allDivs = pf.querySelectorAll('div');
                for (var d = 0; d < allDivs.length; d++) {
                    if (/\bpc\b/.test(allDivs[d].className || '')) {
                        overlay = allDivs[d].cloneNode(true);
                        overlay.querySelectorAll('img').forEach(function(img) { img.remove(); });
                        break;
                    }
                }
            ...
            // Strategy 3: .page-content — find the child div that actually contains spans
            if (!overlay) {
                var pageContent = pf.querySelector('.page-content, [class*="page-content"]');
                if (pageContent) {
                    var children = pageContent.children;
                    for (var c = 0; c < children.length; c++) {
                        if (children[c].querySelectorAll('span').length > 0) {
                            overlay = children[c].cloneNode(true);
                            overlay.querySelectorAll('img').forEach(function(img) { img.remove(); });
                            break;
                        }
                    }
            ...
            // Only include overlay if it actually has visible text spans
            if (overlay && overlay.querySelectorAll('span').length > 0) {
                overlay.style.cssText += ';position:absolute;top:0;left:0;width:100%;height:100%;pointer-events:none;';
                pageOverlays.push(overlay.outerHTML);
            } else {
                pageOverlays.push('');
            }
        }
        ...
        if (!w) {
            ...
            alert('Adblock: Popup blocked! Allow popups for studocu.com.');
        }
    }

    window.StudocuTools = window.StudocuTools || {};
    window.StudocuTools.generatePDF = generatePDF;

    // Button + event delegation
    function createButton() {
        var btn = document.createElement('button');
        btn.classList.add('download-button-1');
        btn.setAttribute('data-adblock', 'download');
    ...
    chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
        if (message?.type !== "STUDOCU_DOWNLOAD_PDF") return false;

        try {
            generatePDF();
            sendResponse({ success: true });
        } catch (error) {
            sendResponse({ success: false, error: error?.message || "PDF_GENERATE_FAILED" });
        }
        return false;
    });
    ...
    document.addEventListener('click', function(e) {
        var btn = e.target.closest('[data-adblock="download"]');
        if (btn) { e.preventDefault(); e.stopPropagation(); e.stopImmediatePropagation(); generatePDF(); }
    }, true);
    document.addEventListener('mousedown', function(e) {
        if (e.target.closest('[data-adblock="download"]')) e.stopPropagation();
    }, true);
